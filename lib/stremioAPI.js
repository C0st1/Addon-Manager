...
async function fetchWithTimeout(url, options = {}, retries = 2, backoff = 1000) {
  ...
  try {
    ...
    if (!res.ok && res.status >= 500 && retries > 0) throw new Error(`HTTP ${res.status}`);
    return res;
  } catch (err) {
    if (retries > 0) {
...
async function cloudSetAddons(addons, authKey) {
  ...
  const compressedBody = await gzip(JSON.stringify({ authKey, addons, type: 'AddonCollectionSet' }));
  const res = await fetchWithTimeout(url, {
    method: 'POST',
